  ...
  return (
    <div className="relative">
      ...
      {(share.status === "ready" || share.status === "error") && (
        <div className="absolute right-0 top-full mt-2 w-72 p-3 bg-white/95 rounded-md shadow-lg text-xs flex flex-col gap-2 z-20">
          {share.status === "ready" ? (
            <>
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded p-1 bg-gray-50 font-mono"
              />
            ...
            <>
              <p className="text-red-600 break-words">{share.error}</p>
              <button
                type="button"
                onClick={() => setShare({ status: "idle" })}
                className="self-end px-2 py-1 rounded border border-gray-300 hover:bg-gray-100"
              >
                Close
              </button>
            </>
